...
public static class Utility
{
    ...
    public static (string prompt, int countdown) ExtractPromptAndCountdown(string rawResponse)
    {
        try
        {
            JObject root = JObject.Parse(rawResponse);
            ...
            if (!string.IsNullOrEmpty(content))
            {
                ...
                string prompt = contentJson["prompt"]?.ToString();
                int countdown = contentJson["countdown"]?.ToObject<int>() ?? 0;

                return (prompt, countdown);
            }
        }
        catch (System.Exception e)
        {
            Debug.LogError("Failed to extract prompt and countdown: " + e.Message);
        }
